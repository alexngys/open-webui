...
from open_webui.retrieval.loaders.valyu import ValyuLoader
...
class SafeValyuLoader(BaseLoader, RateLimitMixin, URLProcessingMixin):
    def __init__(
        self,
        web_paths: Union[str, List[str]],
        api_key: str,
    ...
    def lazy_load(self) -> Iterator[Document]:
        """Load documents with rate limiting support, delegating to ValyuLoader."""
        valid_urls = []
        for url in self.web_paths:
            try:
                self._safe_process_url_sync(url)
...
def get_web_loader(
    ...
    if WEB_LOADER_ENGINE.value == "valyu":
        WebLoaderClass = SafeValyuLoader
        web_loader_args["api_key"] = VALYU_API_KEY.value
        web_loader_args["response_length"] = VALYU_RESPONSE_LENGTH.value
        web_loader_args["extract_effort"] = VALYU_EXTRACT_EFFORT.value
    ...
    if WebLoaderClass:
        ...
        raise ValueError(
            ...
            "Please set it to 'safe_web', 'playwright', 'firecrawl', 'tavily' or 'valyu'."
        )
